# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 12:56:49 2017
...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 15 17:48:31 2017
...
def strategy(sdk):
    ...
    if  tradeDateFlag % HOLDINGPERIOD == 0 :
        ...
        for i in range(-600,0):
            stop = np.array(sdk.getFactorData("LZ_CN_STKA_QUOTE_TCLOSE")[i])
            stopmat.append(stop)
        stopmat = np.transpose(stopmat)
        ...
        for i in range(0,len(stopmat)):
            RS = [0]
            nlist = []
            for  j in range(0,m):
                nn = []
                nn.append(float(math.log(j+1)))
                nlist.append(nn)
                list1 = []
                for ii in range(0,n):
                    list1.append(stopmat[i][j*n+ii])
                ...
                now = 0
                for ii in range(0,n):
                    if max< now+stopmat[i][j*n+ii]-mean:
                        max = now+stopmat[i][j*n+ii]-mean
                    if min > now+stopmat[i][j*n+ii]-mean:
                        min = now+stopmat[i][j*n+ii]-mean
                    now += stopmat[i][j*n+ii]-mean
                RS.append(float(math.log((max-min)/(sd)+0.0001)))  
            
            pp = pd.DataFrame(np.array(RS),columns=['one'])
            pp = pp.fillna(method='pad')
            RS = np.array(pp['one']).tolist()
            RS = RS[1:m+1]
       ...
       # print(len(stopmat[0]))
        ...
        WholeDict=dict(zip(stockList,Hurst))
        ...
        ind = np.array(sdk.getFactorData("LZ_CN_STKA_INDU_ZX")[-1])
        Indict = dict(zip(stockList,ind))
        
        value = np.array(sdk.getFactorData("LZ_CN_STKA_VAL_A_FCAP")[-1])
        valueDict = dict(zip(stockList,value))
        
        prof = np.array(sdk.getFactorData("LZ_CN_STKA_QUOTE_TCLOSE")[-1])-np.array(sdk.getFactorData("LZ_CN_STKA_QUOTE_TCLOSE")[-11])
        profDict = dict(zip(stockList,value))
        
        for key in WholeDict.keys():
            if WholeDict[key]>0.5 and profDict[key]>0: 
                stockToBuy.append(key)
                buy_sq.append(WholeDict[key])
            if WholeDict[key]<0.5 or profDict[key]<0: 
